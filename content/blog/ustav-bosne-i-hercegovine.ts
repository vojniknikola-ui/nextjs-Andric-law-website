...
export const ustavBiHPost: BlogPost = {
  ...
  title: 'Ustav Bosne i Hercegovine – digitalno izdanje sa Amandmanom I',
  excerpt: 'Reobjavljena digitalna verzija Ustava BiH: sidra po članu, jasno označeni dijelovi i integrisan Amandman I za Brčko distrikt.',
  content: `
# Ustav Bosne i Hercegovine – digitalni refresh 2025.

Ponovno smo objavili kompletan tekst Ustava BiH u digitalnom formatu kako bi klijenti i urednici imali čist, visokokontrastni prikaz sa pratećim amandmanom.

## Šta je novo u verziji 2025?
- **Hero u dvije kolone** – odmah se vidi status dokumenta, broj glava i prečice ka ključnim članovima
- **Amandman I kao kartica** – sadržaj iz "Službenog glasnika BiH", br. 25/09 prikazuje se u \`details\` elementu i u samom prikazu
...
3. **Digitalni prikaz** – nije potrebno ručno uređivati HTML; parser automatski prepoznaje \`Član\`, \`GLAVA\`, \`Historijat\` i Markdown naglašavanje  
4. **Pretraga** – svaki novi unos automatski ulazi u semantic-search algoritam (\`/search\`) i dobija badge za propise
